...
non-negativity directly establishes that the optimality term can be discarded or upper-bounded by $0$.

## Main results
* `Online.OCO.LEA.OMD.optimality_nonneg_of_isMinOn`: $0 \le \mathrm{optimality}_t$ whenever $w_{t+1}$
  is a constrained minimizer on $s$ and $u \in s$.
-/
...
lemma optimality_nonneg_of_isMinOn (t : ℕ)
    ...
    (hw : w (t + 1) ∈ s)
    (hu : u ∈ s)
    (h_min : IsMinOn (fun x ↦ η * (g t) x + ψ (t + 1) x - (gψ t (w t)) x) s (w (t + 1))) :
    0 ≤ optimality η gψ u w g t := by
  let lin : E →L[ℝ] ℝ := η • (g t) - gψ t (w t)
  have h_min' : IsMinOn ((ψ (t + 1) + ⇑lin) + fun _ : E ↦ (0 : ℝ)) s (w (t + 1)) := by
    intro x hx
    have := h_min hx
    dsimp [lin] at this ⊢
    simp only [add_zero, sub_apply, smul_apply, smul_eq_mul] at this ⊢
    linarith
  ...
  have h_eq : (gψ (t + 1) (w (t + 1)) + (η • g t - gψ t (w t))) (u - w (t + 1)) =
      ((η : ℝ) • g t + gψ (t + 1) (w (t + 1)) - gψ t (w t)) (u - w (t + 1)) := by
