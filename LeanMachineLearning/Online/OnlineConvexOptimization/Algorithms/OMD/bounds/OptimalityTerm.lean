...
lemma subdifferential_of_isMinOn_fderiv_add {X : Set E} {w : E}
    ...
    (-((gψ_L + lin : E →L[ℝ] ℝ) : E →+ ℝ)) ∈ ∂[X, w] φ := by
  convert (HasFDerivAt.mem_subdifferential_add_iff (g := 0)
    (hψ.add lin.hasFDerivAt) (hψ_conv.add (lin.toLinearMap.convexOn hψ_conv.1)) hφ_conv hw).mp
    (zero_mem_subdifferential_iff_isMinOn.mpr ⟨hw, h_min⟩) using 2
  ext x
...
lemma subgradient_phi_of_isAnchorUpdate {X : Set E} {w : E}
    ...
    (hw : w ∈ X)
    (h_min : IsAnchorUpdate η (g : E →+ ℝ) φ ψ (gψ_anchor : E →+ ℝ) X w) :
    (-(gψ_L : E →+ ℝ) - (η : ℝ) • (g : E →+ ℝ) + (gψ_anchor : E →+ ℝ))
      ∈ ∂[X, w] φ := by
  let lin : E →L[ℝ] ℝ := η • g - gψ_anchor
  have h_min' : IsMinOn ((ψ + ⇑lin) + φ) X w := by
    rwa [show (ψ + ⇑lin) + φ =
        anchorUpdateObjective η (g : E →+ ℝ) φ ψ (gψ_anchor : E →+ ℝ) by
...
lemma firstOrderOptimalityTerm_eq_zero_of_canonical_subgradient
    ...
    firstOrderOptimalityTerm η (g : E →+ ℝ)
      (-(gψ_L : E →+ ℝ) - (η : ℝ) • (g : E →+ ℝ) + (gψ_anchor : E →+ ℝ))
      (gψ_L : E →+ ℝ) (gψ_anchor : E →+ ℝ) u w = 0 := by
  have h_zero : ((η : ℝ) • (g : E →+ ℝ) +
      (-(gψ_L : E →+ ℝ) - (η : ℝ) • (g : E →+ ℝ) + (gψ_anchor : E →+ ℝ)) +
      (gψ_L : E →+ ℝ) - (gψ_anchor : E →+ ℝ)) = 0 := by
    ext x
    simp only [AddMonoidHom.sub_apply, AddMonoidHom.add_apply, AddMonoidHom.neg_apply,
      AddMonoidHom.smul_apply, smul_eq_mul, AddMonoidHom.zero_apply]
    ring
...
lemma subgradient_phi_of_isAnchorUpdate_round (t : ℕ) {X : Set E}
    ...
    (hw : w (t + 1) ∈ X)
    (gψ_anchor : E →L[ℝ] ℝ)
    (hgψ_anchor : gψ t (w_tilde t) = (gψ_anchor : E →+ ℝ))
    (h_min : IsAnchorUpdate η (g t) (φ t) (ψ (t + 1)) (gψ t (w_tilde t)) X (w (t + 1))) :
    (-(gψ_L : E →+ ℝ) - (η : ℝ) • (g t : E →+ ℝ) + (gψ_anchor : E →+ ℝ))
      ∈ ∂[X, w (t + 1)] (φ t) :=
...
lemma subgradient_zero_of_isOptimisticPrediction {X : Set E} {v : E}
    ...
    (hψ_conv : ConvexOn ℝ X ψ)
    (hv : v ∈ X)
    (h_min : IsOptimisticPrediction η (h : E →+ ℝ) ψ (gψ_anchor : E →+ ℝ) X v) :
    (-(gψ_L : E →+ ℝ) - (η : ℝ) • (h : E →+ ℝ) + (gψ_anchor : E →+ ℝ))
      ∈ ∂[X, v] (0 : E → ℝ) := by
...
lemma optimisticOptimalityTerm_eq_zero_of_canonical_gradient (t : ℕ)
    (gψ_anchor : E →L[ℝ] ℝ)
    (hgψ_anchor : gψ t (w_tilde t) = (gψ_anchor : E →+ ℝ))
    (hgψ : gψ (t + 1) (v t) = (-(η : ℝ) • (h t : E →+ ℝ) + (gψ_anchor : E →+ ℝ))) :
    optimisticOptimalityTerm η gψ w w_tilde v (fun n ↦ h n) t = 0 := by
  have h_zero : ((η : ℝ) • (h t : E →+ ℝ) + 0 +
      gψ (t + 1) (v t) - gψ t (w_tilde t)) = 0 := by
    ext x
    rw [hgψ, hgψ_anchor]
    simp only [AddMonoidHom.sub_apply, AddMonoidHom.add_apply, AddMonoidHom.zero_apply,
...
lemma optimisticOptimalityTerm_nonneg_of_isOptimisticPrediction (t : ℕ) {X : Set E}
  ...
  have h_eval := h_sub.2 (w (t + 1)) hw
  dsimp [optimisticOptimalityTerm]
  rw [hgψ, hgψ_anchor]
  have h_eq : firstOrderOptimalityTerm η (h t : E →+ ℝ) 0 (gψ_L : E →+ ℝ) (gψ_anchor : E →+ ℝ)
      (w (t + 1)) (v t) =
      D_[0](w (t + 1), v t,
        -(gψ_L : E →+ ℝ) - (η : ℝ) • (h t : E →+ ℝ) + (gψ_anchor : E →+ ℝ)) := by
