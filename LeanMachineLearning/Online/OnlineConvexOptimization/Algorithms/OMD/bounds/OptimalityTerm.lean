...
when the iterate $w_{t+1}$ minimizes the anchor update objective over a convex constraint
...
lemma optimalityTerm_eq_zero_of_canonical_subgradient (t : ℕ)
    ...
    (hgψ_anchor : gψ t (w_tilde t) = gψ_anchor)
    (hgφ : gφ t (w (t + 1)) =
      (-gψ_L - η • (g t) + gψ_anchor + gψ (t + 1) (w_tilde 1) - gψ t (w_tilde 1))) :
